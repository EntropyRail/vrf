    ...
    function hashToPoint(bytes memory dst, bytes memory message) internal view returns (PointG1 memory out) {
        ...
        for (uint256 i = 0; i < 2; i++) {
            assembly {
                ...
                q := add(q, 32)
                // Equivalent to MCOPY for 64 non-overlapping bytes, while keeping the
                // coordinator build compatible with its pinned Shanghai EVM target.
                mstore(q, mload(p))
                mstore(add(q, 32), mload(add(p, 32)))
                q := add(q, 64)
